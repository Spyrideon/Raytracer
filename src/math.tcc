...
template <class FLOAT_TYPE, size_t N>
FLOAT_TYPE Vector<FLOAT_TYPE, N>::at(std::size_t i) const {
...
FLOAT_TYPE Vector<FLOAT_TYPE, N>::square_of_length() const {
  FLOAT_TYPE sum = static_cast<FLOAT_TYPE>(0);
  for (size_t i = 0u; i < N; ++i) sum += vector[i] * vector[i];
  return sum;
}
...
F operator*(Vector<F, K> v1, const Vector<F, K> v2) {
  F sum = static_cast<F>(0);
  for (size_t i = 0u; i < K; ++i) sum += v1.vector[i] * v2.vector[i];
  return sum;
}